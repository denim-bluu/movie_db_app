...
sudo -i -u postgres psql -c 'CREATE DATABASE movie'
sudo -i -u postgres psql -d movie -c 'CREATE EXTENSION IF NOT EXISTS citext'
sudo -i -u postgres psql -d movie -c "CREATE ROLE movie WITH LOGIN PASSWORD '${DB_PASSWORD}'"

# Add a DSN for connecting to the greenlight database to the system-wide environment 
# variables in the /etc/environment file.
echo "MOVIE_DB_DSN='postgres://movie:${DB_PASSWORD}@localhost/movie'" >> /etc/environment
